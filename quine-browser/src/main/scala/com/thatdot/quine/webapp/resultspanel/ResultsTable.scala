...
object ResultsTable {
  ...
  def apply(
    ...
    table(
      ...
      thead(tr(result.columns.zipWithIndex.map { case (name, idx) =>
        headerCell(name, idx, sortCol, sortDir, colWidths, () => ColumnResize.measuredWidths(tableRef), vd)
      })),
      tbody(
        result.results.map { row =>
          tr(
            onClick --> (_ => vd.onNext(ViewerCommand.SelectRow(result.columns.zip(row)))),
  ...
  private def headerCell(
    ...
    th(
      ...
      title <-- sortCol.combineWith(sortDir).map {
        case (Some(c), SortDir.Asc) if c == colIdx => "Sort descending"
        case _ => "Sort ascending"
      },
      span(name),
      span(
