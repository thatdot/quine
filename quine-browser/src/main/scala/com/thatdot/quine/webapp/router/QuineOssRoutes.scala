...
class QuineOssRoutes(apiV1: Boolean, basePath: String) {
  ...
  private val dashboardRoute: Route.Total[Landing.type, Unit] =
    Route.static(staticPage = Landing, pattern = root / "dashboard", basePath = basePath)

  private val explorationUiRoute: Route.Total[ExplorerUi.type, Unit] =
    Route.static(staticPage = ExplorerUi, pattern = root / "explorer", basePath = basePath)
  ...
  private val metricsRoute: Route.Total[Metrics.type, Unit] =
    Route.static(staticPage = Metrics, pattern = root / "metrics", basePath = basePath)
