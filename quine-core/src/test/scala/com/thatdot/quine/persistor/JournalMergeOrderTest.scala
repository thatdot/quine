...
/** Pins the order [[PersistenceAgent.getJournalWithTime]] merges the two journal stores in.
  ...
  * interleaved changes what a node wakes up as, so the merge is pinned here against the exact
  * expression it has always computed: a stable sort of (node-change events ++ domain-index events)
  * by millisecond.
  */
class JournalMergeOrderTest extends AnyFunSuite with BeforeAndAfterAll with should.Matchers {
  ...
  /** The merge order this must always produce: a stable sort by millisecond of the node-change
    * events followed by the domain-index events. Ties therefore resolve to node-change first, and
    * each store's internal order is preserved.
    */
  private def expectedMerge(
    ...
    (nodeChanges ++ domainIndex).sortBy(e => e.atTime.millis)
  ...
  test("a domain-index event ties with node-change events in the same millisecond, and loses") {
    // Every event in one millisecond: the index event is stamped between the two property events,
    // but a stable sort by millisecond hoists both property events ahead of it.
    ...
    result.map(_.event) shouldEqual Vector(first.event, second.event, index.event)
  }
  ...
  test("randomised interleavings all match a stable sort of the concatenation") {
  ...
  /** Assign each millisecond in `millis` a distinct [[EventTime]], ticking the sequence counter
    * when a millisecond repeats. A node stamps its events this way, so a store never holds two
    * events at the same EventTime even when it holds several in the same millisecond.
    */
  ...
  private def forAllInterleavings(random: Random): Unit =
    (1 to 40).foreach { case _ =>
      ...
      val nodeChanges =
        distinctTimes(Vector.fill(random.nextInt(6))(random.nextInt(4).toLong * 10L).sorted).zipWithIndex
          .map { case (t, i) => propertyEvent(s"p$i", t) }
      val domainIndex =
        distinctTimes(Vector.fill(random.nextInt(6))(random.nextInt(4).toLong * 10L).sorted).zipWithIndex
          .map { case (t, i) => indexEvent(i.toLong, t) }
