...
object StandingQueryPattern {
  ...
  object StandingQueryMode {
    ...
    // Decoder accepts all values (including internal QuinePattern); schema exposes only publicValues
    // so QuinePattern doesn't surface in the OpenAPI enum list.
    implicit val encoder: Encoder[StandingQueryMode] = ScreamingSnakeEnum.encoder
    implicit val decoder: Decoder[StandingQueryMode] = ScreamingSnakeEnum.decoder(values)
    implicit lazy val schema: Schema[StandingQueryMode] = ScreamingSnakeEnum.schema(publicValues)
  }
}
