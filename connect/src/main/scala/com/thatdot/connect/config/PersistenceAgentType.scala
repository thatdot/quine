...
import com.thatdot.connect.Metrics
...
object PersistenceAgentType {
        ...
        try new RocksDbPersistor(
          ...
          case err: UnsatisfiedLinkError =>
            logger.error(
              ...
              "Consider using MapDB instead by specifying `thatdot.connect.store.type=map-db`",
              err
            )
            sys.exit(1)
        }
  ...
  final case class Cassandra(
    keyspace: String = sys.env.getOrElse("CASSANDRA_KEYSPACE", "thatdot"),
