...
import com.thatdot.connect.{BuildInfo, Config}
import com.thatdot.quine.graph._
import com.thatdot.quine.gremlin.GremlinQueryRunner
import com.thatdot.quine.model.{QuineId, QuineIdProvider}

/** Main webserver route
  *
  *   - Query UI page (along with assets, JS libaries it depends on)
  *   - API endpoints
  *   - OpenAPI page documenting the API endpoints
  *   - Bolt protocol WS handler
  */
class ConnectRoutes(
  ...
  lazy val staticFilesRoute: Route = {
    Directives.pathEndOrSingleSlash {
      getFromResource("web/connect-ui.html")
    } ~
    Directives.path("dashboard" | "docs") {
      getFromResource("web/connect-ui.html")
    } ~
    Directives.path("connect-ui-startup.js") {
      getFromResource("web/connect-ui-startup.js")
    } ~
  ...
  /** OpenAPI route */
  lazy val openApiRoute: Route = ConnectOpenApiDocsRoutes(graph).route
