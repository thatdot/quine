...
import com.thatdot.connect.importers.createIngestStream
import com.thatdot.connect.routes.{
...
/** The `connect` application and its associated state
  *
  * @param graph reference to the underlying graph
  */
final class ConnectAppState(graph: GraphService)
    extends BaseAppState(graph)
  ...
  def setSampleQueries(newSampleQueries: Vector[SampleQuery]): Future[Unit] = {
    ...
    storeClusterMetaData(SampleQueriesKey, sampleQueries)
  }

  def setQuickQueries(newQuickQueries: Vector[UiNodeQuickQuery]): Future[Unit] = {
    quickQueries = newQuickQueries
    storeClusterMetaData(QuickQueriesKey, quickQueries)
  }

  def setNodeAppearances(newNodeAppearances: Vector[UiNodeAppearance]): Future[Unit] = {
    nodeAppearances = newNodeAppearances
    storeClusterMetaData(NodeAppearancesKey, nodeAppearances)
  }
  type FriendlySQName = String
  type SQOutputName = String
  /*
  type StandingQueryOutputTarget =
    (FriendlySQName, (StandingQueryId, Map[SQOutputName, StandingQueryResultOutputUserDef]))
  type SQClusterSerialization = (FriendlySQName, StandingQueryId, StandingQueryDefinition, QuineHost)

  private val sqIdSchema: JsonSchema[StandingQueryId] = genericJsonSchema[StandingQueryId]
  private val quineHostSchema: JsonSchema[QuineHost] = genericJsonSchema[QuineHost]

  private val addStandingQuerySchema: JsonSchema[SQClusterSerialization] =
    tuple4JsonSchema(implicitly[JsonSchema[FriendlySQName]], sqIdSchema, standingQuerySchema, quineHostSchema)
  private val cancelStandingQuerySchema: JsonSchema[(String, QuineHost)] =
    tuple2JsonSchema(defaultStringJsonSchema, quineHostSchema)
   */
  ...
  /** Get standing queries live on the cluster with the specified names
    *
    ...
    ingestStreams.foreach {
      case (_, streamWithControl) if streamWithControl.restored =>
        streamWithControl.valve.map { v =>
          // In Connect, this future is always created with `Future.success(…)`. This is ugly.
...
object ConnectAppState {

  import ApiConverters._

  final val VersionKey = "connect_app_state_version"
  ...
  /** Version to track schemas saved by Connect app state
    *
    * Remember to increment this if schemas in Connect app state evolve in
    * backwards incompatible ways.
    */
  final val CurrentPersistenceVersion: Version = Version(1, 0, 0)

  def connectAppStateIsEmpty(persistenceAgent: PersistenceAgent)(implicit ec: ExecutionContext): Future[Boolean] = {
    val metaDataKeys =
  ...
  /** Aggregate Connect SQ outputs and Quine standing query into a user-facing SQ
    *
